...
def run():
    ...
    while True:
        ...
        if user_input.lower() in {"quit", "exit", "bye"}:
            print("Goodbye!")
            break
        # appends each user message to the conversation hist
        # ory as a HumanMessage object, which is the format expected by the LLM
        history.append(HumanMessage(content=user_input))
        history = history[-5:]  # keep last 5 messages to prevent context overflow  (to save tokens and ensure relevant context)
        print("\nAgent: ", end="", flush=True)
        final_messages = None

        try:
            # LangGraph starts executing your workflow
